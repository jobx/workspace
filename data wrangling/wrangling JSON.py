# -*- coding: utf-8 -*-
"""
Created on Sun Aug 15 15:30:59 2021
...
def get_from_file(kind, period):
    ...
    filepath = os.path.join(os.getcwd() + '\documents', filename)
...
def article_overview(kind, period):
    ...
    records = get_from_file(kind, period)
    titles = []
    urls =[]
    # YOUR CODE HERE
    for record in records:
        # record is a dictionary
        title = {record['section']:record['title']}
        titles.append(title)
        
        medias = record['media']
        for media in medias:
            for data in media['media-metadata']:
                if data['format'] == 'Standard Thumbnail':
                    url = data['url']
                    urls.append(url)
